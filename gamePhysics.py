...
class GamePhysic:
    ...
    def collide(self, entity_position, entity_size) -> str:
        ...
        for wall_collide in self.data_base.walls_collide:
            ...
            pos_center_wall_x = wall_position[0] / 2
            pos_center_wall_y = wall_position[1] / 2

            # Calculer le centre en x et y de l'entité
            pos_center_entity_x = entity_position[0] / 2
            pos_center_entity_y = entity_position[1] / 2

            if True:  # """""""""""""""""""""""""""""Vérifier la distance entre le mur et l'entité ici"""""""""""""""""""""""""""""
                """quand le mur se trouve est suffisamment proche du joueur"""

                # print(f"pos: {wall_position}; center: {pos_center_wall_x + wall_size[0], pos_center_wall_y + wall_size[1]}")

                # vérification de si l'entité touche le mur
                if entity_position[0] < (wall_position[0] + (wall_size[0] / 2)):
                    """quand l'entité se trouve a droite"""
                    distance_center = pos_center_wall_x + pos_center_entity_x

                    if 0 <= distance_center <= 10:
                        zone_collide.append("right")

                if (entity_position[0] + (entity_size[0] / 2)) > wall_collide[0]:
                    """quand l'entité se trouve a gauche"""
                    distance_center = (entity_position[0] + entity_size[0]) - pos_center_wall_x

                    if -6 <= distance_center <= 6:
                        zone_collide.append("left")

                if (entity_position[1] + ((entity_size[1] / 2)) < wall_position[1]):
                    """quand l'entité se trouve en haut"""
                    distance_center = pos_center_wall_y - pos_center_entity_y

                    if -6 <= distance_center - entity_size[1] <= 6:
                        zone_collide.append("top")

                if entity_position[1] > (wall_position[1] + (wall_size[1] / 2)):
                    """quand l'entité se trouve en bas"""
                    distance_center = pos_center_wall_y + pos_center_entity_y

                    if -6 <= distance_center + wall_size[1] <= 6:
